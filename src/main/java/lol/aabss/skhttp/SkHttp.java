...
public final class SkHttp extends JavaPlugin {
    ...
    public void onEnable() {
        if (Bukkit.getPluginManager().getPlugin("Skript") != null) {
            // Проверка минимальной версии Skript
            if (!Skript.isAcceptRegistrations()) {
                LOGGER.error("Skript is not ready for addon registration!");
                getPluginLoader().disablePlugin(this);
                return;
            }
            
            // Проверка версии Skript (2.11+)
            String skriptVersion = Skript.getVersion().toString();
            if (!isSkriptVersionSupported(skriptVersion)) {
                LOGGER.error("SkHttp requires Skript 2.11 or higher! Current version: " + skriptVersion);
                getPluginLoader().disablePlugin(this);
                return;
            }
...
/**
     * Проверяет, поддерживается ли версия Skript (2.11+)
     */
    private boolean isSkriptVersionSupported(String version) {
        try {
            // Убираем префиксы и суффиксы версии
            ...
            if (parts.length >= 2) {
                ...
                // Проверяем версию 2.11+
                return major > 2 || (major == 2 && minor >= 11);
            }
        } catch (NumberFormatException e) {
            LOGGER.error("Failed to parse Skript version: " + version);
        }
        return false;
    }
